...
@objc extension URLSessionConfiguration {
    ...
    @objc private class var default_swizzled: URLSessionConfiguration {
        get {
            ...
            // Let's go ahead and add in AGTProtocol, since it's safe to do so.
            config.protocolClasses?.insert(AGTProtocol.self, at: 0)
...
struct AGTPath {
    ...
    static let AGTDirURL = tmpDirURL.appendingPathComponent("AGT", isDirectory: true)
    static let sessionLogURL = AGTDirURL.appendingPathComponent(sessionLogName)

    static func createAGTDirIfNotExist() {
        do {
            try FileManager.default.createDirectory(at: AGTDirURL, withIntermediateDirectories: true, attributes: nil)
        } catch let error {
            print("[AGT]: failed to create working dir - \(error.localizedDescription)")
        }
    }

    static func deleteAGTDir() {
        guard FileManager.default.fileExists(atPath: AGTDirURL.path, isDirectory: nil) else { return }

        do {
            try FileManager.default.removeItem(at: AGTDirURL)
        } catch let error {
            print("[AGT]: failed to delete working dir - \(error.localizedDescription)")
        }
    }

    static func deleteOldAGTLogs() {
        let oldSessionLogName = "session.log"
        let oldRequestPrefixName = "AGT_re"
        let fileManager = FileManager.default
        guard let documentsDir = fileManager.urls(for: .documentDirectory, in: .userDomainMask).first,
              let fileEnumaAGTor = fileManager.enumerator(at: documentsDir, includingPropertiesForKeys: nil, options: [.skipsSubdirectoryDescendants], errorHandler: nil) else { return }

        for case let fileURL as URL in fileEnumaAGTor {
    ...
    static func pathURLToFile(_ fileName: String) -> URL {
        return AGTDirURL.appendingPathComponent(fileName)
    }

}

extension URLRequest {
    func getAGTURL() -> String {
        if (url != nil) {
    ...
    func getAGTURLComponents() -> URLComponents? {
    ...
    func getAGTTimeout() -> String {
        return String(Double(timeoutInterval))
    }

    func getAGTHeaders() -> [AnyHashable: Any] {
    ...
    func getAGTBody() -> Data {
        return httpBodyStream?.readfully() ?? URLProtocol.property(forKey: "AGTBodyData", in: self) as? Data ?? Data()
    }
    ...
    func getCurl() -> String {
        ...
        for (key, value) in getAGTHeaders() {
            command.append("-H \u{22}\(key): \(value)\u{22}")
        }

        if let body = String(data: getAGTBody(), encoding: .utf8) {
...
extension String {
    ...
    func appendToFileURL(_ fileURL: URL) {
        ...
        if #available(iOS 13.4, *) {
            do {
                ...
                print("[AGT]: Failed to append [\(self.prefix(128))] to \(fileURL), trying to create new file - \(error.localizedDescription)")
                write(to: fileURL)
            }
        } else {
    ...
    private func write(to fileURL: URL) {
        do {
            ...
            print("[AGT]: Failed to save [\(self.prefix(128))] to \(fileURL) - \(error.localizedDescription)")
        }
    }
...
extension UIWindow {
    ...
    override open func motionEnded(_ motion: UIEvent.EventSubtype, with event: UIEvent?) {
        if AGT.sharedInstance().getSelectedGesture() == .shake {
            if (event!.type == .motion && event!.subtype == .motionShake) {
                AGT.sharedInstance().motionDetected()
            }
        } else {
